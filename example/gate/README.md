...
本文主要介绍如何通过框架SDK开发 [Gate.io](https://gateio.news) 交易所的交易系统。
...
- 注册 [Gate.io](https://gateio.news) 的账户，并且创建 `ACCESS KEY` 和 `SECRET KEY`，AK有操作委托单权限；
...
- PLATFORMS 指定需要使用的交易账户，注意名字是 `gate` ；
- strategy 策略的名称；
- symbol 策略运行的交易对；
